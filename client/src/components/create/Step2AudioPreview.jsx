...
const Step2AudioPreview = ({ data, setData, onNext, onPrev }) => {
        ...
        const renderStatusBadge = (status) => {
                  ...
                  if (status === 'generating') {
                              return <span className="px-2 py-1 text-xs rounded-full bg-blue-100 text-blue-600 flex items-center gap-1"><RefreshCw className="w-3 h-3 animate-spin" />生成中</span>span>;
                  }
                  if (status === 'ready') {
                              return <span className="px-2 py-1 text-xs rounded-full bg-green-100 text-green-600 flex items-center gap-1"><CheckCircle className="w-3 h-3" />已确认</span>span>;
                  }
                  if (status === 'error') {
