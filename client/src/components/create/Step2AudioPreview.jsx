...
const Step2AudioPreview = ({ data, setData, onNext, onPrev }) => {
          ...
          const handleGenerateSegmentAudio = async (segmentId) => {
                      ...
                      try {
                                    ...
                                    const updatedSegments = segments.map(s => s.id === segmentId ? { ...s, status: 'ready', audioUrl, audioBlob } : s);
          ...
          const renderStatusBadge = (status) => {
                      if (status === 'pending') return <span className="px-2 py-1 text-xs rounded-full bg-gray-100 text-gray-600">待生成</span>span>;
                      if (status === 'generating') return <span className="px-2 py-1 text-xs rounded-full bg-blue-100 text-blue-600 flex items-center gap-1"><RefreshCw className="w-3 h-3 animate-spin" />生成中</span>span>;
                      if (status === 'ready') return <span className="px-2 py-1 text-xs rounded-full bg-green-100 text-green-600 flex items-center gap-1"><CheckCircle className="w-3 h-3" />已确认</span>span>;
                      if (status === 'error') return <span className="px-2 py-1 text-xs rounded-full bg-red-100 text-red-600">生成失败</span>span>;
                      return null;
          };
          ...
          return (
                      <div className="max-w-7xl mx-auto p-6">
                            <div className="mb-6">
                                    <h2 className="text-2xl font-bold text-gray-900 mb-2">🎤 Step 2: 逐段试听确认</h2>h2>
                                    <p className="text-gray-600">选择声音并逐段生成试听，确认每段效果满意后进入下一步</p>p>
                            </div>div>
                            ...
                            <div className="grid grid-cols-3 gap-6">
                                    <div className="space-y-4">
                                              <h3 className="text-lg font-semibold flex items-center gap-2"><Volume2 className="w-5 h-5" />选择声音</h3>h3>
                                            ...
                                            <div className="ml-6 space-y-3">
                                                            ...
                                                            <p className="text-xs text-gray-500">支持WAV、MP3、M4A格式，最大50MB</p>p>
                                                    {userVoices.length > 0 && (
                                                                              ...
                                                                              {userVoices.map(voice => (
                                                                                                    <label key={voice.voiceId} className={'flex items-center gap-3 p-3 border rounded-lg cursor-pointer transition-all ' + (customVoiceId === voice.voiceId ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-blue-200')}>
                                                                                                                            ...
                                                                                                                            <div className="flex-1">
                                                                                                                                                      ...
                                                                                                                                                      <div className="text-xs text-gray-500">{voice.duration ? voice.duration.toFixed(1) + '秒' : ''} • {new Date(voice.createdAt).toLocaleDateString()}</div>div>
                                                                                                                                    </div>div>
                                                                                                            </label>label>
                                                                                                  ))}
                                                                      </div>div>
                                              ...
                                              <div className="p-4 border border-gray-200 rounded-lg">
                                                          ...
                                                          <div className="space-y-3">
                                                                        <div>
                                                                                        <label className="flex justify-between text-sm mb-1"><span>语速</span>span><span className="text-blue-600">{voiceSettings.speed.toFixed(1)}x</span>span></label>label>
                                                                        ...
                                                                        <div>
                                                                                        <label className="flex justify-between text-sm mb-1"><span>音调</span>span><span className="text-blue-600">{voiceSettings.pitch.toFixed(1)}x</span>span></label>label>
                                                                        ...
                                                                        <div>
                                                                                        <label className="flex justify-between text-sm mb-1"><span>音量</span>span><span className="text-blue-600">{(voiceSettings.volume * 100).toFixed(0)}%</span>span></label>label>
                                                                        ...
                                                                        <div className="space-y-3">
                                                                                        <div>
                                                                                                          <label className="flex justify-between text-sm mb-1">
                                                                                                                              <span>😊 快乐</span>span>
                                                                                                                              <span className="text-blue-600">{Math.round((voiceSettings.emotions?.happiness || 0.7) * 100)}%</span>span>
                                                                                                                  </label>label>
                                                                                        ...
                                                                                        <div>
                                                                                                          <label className="flex justify-between text-sm mb-1">
                                                                                                                              <span>😢 悲伤</span>span>
                                                                                                                              <span className="text-blue-600">{Math.round((voiceSettings.emotions?.sadness || 0.1) * 100)}%</span>span>
                                                                                                                  </label>label>
                                                                                        ...
                                                                                        <div>
                                                                                                          <label className="flex justify-between text-sm mb-1">
                                                                                                                              <span>😠 愤怒</span>span>
                                                                                                                              <span className="text-blue-600">{Math.round((voiceSettings.emotions?.anger || 0) * 100)}%</span>span>
                                                                                                                  </label>label>
                                                                                        ...
                                                                                        <div>
                                                                                                          <label className="flex justify-between text-sm mb-1">
                                                                                                                              <span>😮 惊讶</span>span>
                                                                                                                              <span className="text-blue-600">{Math.round((voiceSettings.emotions?.surprise || 0.3) * 100)}%</span>span>
                                                                                                                  </label>label>
                                    ...
                                    <div className="col-span-2 space-y-4">
                                              <div className="flex items-center justify-between">
                                                          <h3 className="text-lg font-semibold flex items-center gap-2"><Music className="w-5 h-5" />分段试听<span className="text-sm font-normal text-gray-500">（共 {segments.length} 段）</span>span></h3>h3>
                                            ...
                                            <div key={segment.id} className={'p-4 border rounded-lg transition-colors ' + (currentPlayingSegment === segment.id ? 'border-blue-500 bg-blue-50' : 'border-gray-200 bg-white hover:border-blue-300')}>
                                                            <div className="flex items-center justify-between mb-3">
                                                                              <div className="flex items-center gap-3">
                                                                                                  <span className="inline-flex items-center justify-center w-8 h-8 bg-blue-100 text-blue-600 rounded-full text-sm font-semibold">{index + 1}</span>span>
                                                            ...
                                                            <div className="p-3 bg-gray-50 rounded-lg text-sm text-gray-700 leading-relaxed">{segment.text}</div>div>
                                            </div>div>
                                          ))}
                                              </div>div>
                                              ...
                                              <div className="p-4 bg-gradient-to-r from-blue-50 to-green-50 rounded-lg border border-blue-200">
                                                          <div className="flex items-center justify-between">
                                                                        <div className="flex items-center gap-4">
                                                                                        <div className="text-sm"><span className="text-gray-600">已确认: </span>span><span className="font-semibold text-green-600">{segments.filter(s => s.status === 'ready').length}</span>span><span className="text-gray-600"> / {segments.length} 段</span>span></div>div>
                                                                                        <div className="text-sm"><span className="text-gray-600">总字数: </span>span><span className="font-semibold text-blue-600">{segments.reduce((sum, s) => sum + s.text.length, 0)} 字</span>span></div>div>
                                                                        </div>div>
                                                                  {allSegmentsReady && (<span className="flex items-center gap-2 text-green-600 font-semibold"><CheckCircle className="w-5 h-5" />全部确认完成！</span>span>)}
                                                          </div>div>
                                              </div>div>
                                    </div>div>
                            </div>div>
                            ...
                            <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                                    <p className="text-sm text-blue-800">💡 <strong>提示：</strong>strong>点击每段右侧的生成试听按钮，逐段生成语音并试听。如果对某段效果不满意，可以点击重新生成直到满意为止。所有分段确认后，才能进入下一步生成视频。</p>p>
                            </div>div>
                      
                            <div className="flex justify-between pt-6">
                                    <button type="button" onClick={onPrev} className="px-6 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">← 上一步</button>button>
                                    <button type="button" onClick={handleNext} disabled={!allSegmentsReady} className="flex items-center gap-2 px-6 py-2 bg-gradient-to-r from-green-600 to-green-700 text-white rounded-lg hover:from-green-700 hover:to-green-800 disabled:from-gray-300 disabled:to-gray-300 transition-all">下一步：选择视频模板<ArrowRight className="w-4 h-4" /></button>button>
                            </div>div>
                      </div>div>
                    );
};
