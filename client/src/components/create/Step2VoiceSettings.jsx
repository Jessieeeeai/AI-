...
export default function Step2VoiceSettings({ data, updateData, onNext, onPrev }) {
  ...
  // 修复：统一使用后端TTS API生成预览（预设声音和自定义声音都调用后端）
  const handlePreview = async () => {
        ...
        try {
          ...
          // 确定 voiceId：
          // - 自定义声音：使用上传的 voiceId
          // - 预设声音：传 null，后端会使用 'default'
          const voiceId = useCustomVoice && uploadedVoice?.voiceId
                  ? uploadedVoice.voiceId
                    : null;

          console.log('🎤 生成声音预览:', {
                    voiceId: voiceId || 'default (预设)',
