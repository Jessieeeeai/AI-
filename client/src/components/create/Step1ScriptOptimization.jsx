...
const Step1ScriptOptimization = ({ data, setData, onNext }) => {
        ...
        const handleOptimize = async () => {
                  if (!originalText || originalText.trim().length < 10) { setError('请输入至少10个字符的文案'); return; }
                  ...
                  } catch (err) { setError(err.message || '文案优化失败，请重试'); }
                  finally { setIsOptimizing(false); }
        };

        const handleManualEdit = (text) => { setOptimizedText(text); setSegments([]); setData({ ...data, optimizedText: text, originalText, segments: [] }); };
        const handleReSegment = () => {
                  if (!optimizedText || optimizedText.trim().length < 10) { setError('请先输入或优化文案'); return; }
        ...
        const handleNext = () => {
                  if (!optimizedText || optimizedText.trim().length < 10) { setError('请先优化文案或手动输入口播稿'); return; }
                  if (segments.length === 0) { setError('请先进行智能分段'); return; }
                  onNext();
        };
        const handleClear = () => { setOriginalText(''); setOptimizedText(''); setOptimizationStats(null); setSegments([]); setError(''); };
        ...
        const renderStyleButton = (value, label, desc) => {
                  return (
                              <button key={value} type="button" onClick={() => setStyle(value)} className={'flex-1 p-4 rounded-lg border-2 transition-all ' + (style === value ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-blue-200')}>
                                            <div className="font-semibold">{label}</div>div>
                                      <div className="text-sm text-gray-500">{desc}</div>div>
                              </button>button>
                            );
        };
                  ...
                  <div className="max-w-7xl mx-auto p-6">
                        ...
                        <div className="mb-6">
                                ...
                                <div className="flex gap-3">
                                      {renderStyleButton('humorous', '🎭 风趣幽默', '轻松活泼')}
                                      {renderStyleButton('professional', '💼 专业严谨', '客观中立')}
                                      {renderStyleButton('casual', '😊 轻松随意', '亲切自然')}
                                </div>div>
                        </div>div>
                        ...
                        <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-4">
                                          ...
                                          <textarea value={originalText} onChange={(e) => setOriginalText(e.target.value)} placeholder="粘贴您的文章或文案..." className="w-full h-80 p-4 border border-gray-300 rounded-lg resize-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" maxLength={10000} />
                                          <div className="flex gap-3">
                                                      <button type="button" onClick={handleClear} className="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">清空</button>button>
                                                      <button type="button" onClick={handleOptimize} disabled={isOptimizing || !originalText} className="flex-1 flex items-center justify-center gap-2 px-6 py-2 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg hover:from-blue-700 hover:to-blue-800 disabled:from-gray-300 disabled:to-gray-300 disabled:cursor-not-allowed transition-all">
                                                            {isOptimizing ? <><RefreshCw className="w-4 h-4 animate-spin" />正在优化...</>> : <><Sparkles className="w-4 h-4" />AI优化并分段</>>}
                                                      </button>button>
                                          </div>div>
                                </div>div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <>
                                                  <textarea value={optimizedText} onChange={(e) => handleManualEdit(e.target.value)} className="w-full h-80 p-4 border border-gray-300 rounded-lg resize-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" maxLength={10000} />
                                          {optimizationStats && (
                                                                                                ...
                                                                                                <div>
                                                                                                                      ...
                                                                                                                      <div className="mt-3 pt-3 border-t border-green-300">
                                                                                                                                              <div className="flex justify-between items-center">
                                                                                                                                                                        <div><span className="text-sm text-gray-600">预计时长: </span>span><span className="font-semibold text-blue-600">{Math.floor(estimatedDuration / 60)}分{estimatedDuration % 60}秒</span>span></div>div>
                                                                                                                                                                        <div><span className="text-sm text-gray-600">预计消耗: </span>span><span className="font-semibold text-orange-600">{estimatedCredits} 积分</span>span></div>div>
                                                                                                                                                    </div>div>
                                                                                                                            </div>div>
                                                                                                      </div>div>
                                                                            </div>div>
                                ...
                                <div className="mt-8">
                                          <div className="flex items-center justify-between mb-4">
                                                      <h3 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
                                                                    <Scissors className="w-5 h-5" />智能分段预览<span className="text-sm font-normal text-gray-500">（共 {segments.length} 段，约300字/段）</span>span>
                                                      </h3>h3>
                                                      <button type="button" onClick={handleReSegment} disabled={isSegmenting} className="flex items-center gap-2 px-4 py-2 border border-blue-500 text-blue-600 rounded-lg hover:bg-blue-50 transition-colors">
                                                                    <RefreshCw className={'w-4 h-4 ' + (isSegmenting ? 'animate-spin' : '')} />重新分段
                                                      </button>button>
                                          </div>div>
                                                  ...
                                                  <div className="space-y-4">
                                                        {segments.map((segment, index) => (
                                                                        <div key={segment.id} className="p-4 border border-gray-200 rounded-lg bg-white hover:border-blue-300 transition-colors">
                                                                                          ...
                                                                                          <textarea value={segment.text} onChange={(e) => handleSegmentEdit(segment.id, e.target.value)} className="w-full p-3 border border-gray-200 rounded-lg resize-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm" rows={3} />
                                                                        </div>div>
                                                                      ))}
                                                  </div>div>
                                          )}
                        ...
                        {optimizedText && segments.length > 0 && (
                                <div className="mt-6 flex gap-3 justify-end">
                                          <button type="button" onClick={handleOptimize} disabled={isOptimizing} className="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors flex items-center gap-2">
                                                      <RefreshCw className="w-4 h-4" />重新优化
                                          </button>button>
                                          <button type="button" onClick={handleNext} className="flex items-center justify-center gap-2 px-6 py-2 bg-gradient-to-r from-green-600 to-green-700 text-white rounded-lg hover:from-green-700 hover:to-green-800 transition-all">
                                                      下一步：逐段试听确认<ArrowRight className="w-4 h-4" />
                                          </button>button>
                                </div>div>
                        )}
                  
                        <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                                <p className="text-sm text-blue-800">💡 <strong>智能分段说明：</strong>strong>系统会自动将文案按约300字分段，并在句子结尾处断开，确保每段内容完整流畅。分段后您可以手动调整每段内容，下一步将逐段生成语音并试听。</p>p>
                        </div>div>
                  </div>div>
                );
};
