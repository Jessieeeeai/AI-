...
class VideoGenerationService {
    constructor() {
          ...
          console.log(`🎬 VideoGenerationService初始化 | 模式: ${this.useMock ? 'Mock (CPU)' : 'Real (GPU)'} | RunPod: ${this.useRunPodServerless ? '启用' : '禁用'}`);
    }
    ...
    /**
       * 生成TTS音频
          * 支持直接调用IndexTTS2 API或通过RunPod Serverless调用
    ...
    async generateTTS(text, voiceId = 'default', outputPath = null) {
          try {
                  console.log(`🎤 生成TTS | 文本长度: ${text.length} | 声音: ${voiceId} | RunPod: ${this.useRunPodServerless}`);

                  let audioData;

                  if (this.useRunPodServerless) {
                            // 使用RunPod Serverless API
                            console.log('📡 通过RunPod Serverless调用TTS...');
                            ...
                            // RunPod返回的是base64编码的音频数据
                            if (result.audio_base64) {
                                        audioData = Buffer.from(result.audio_base64, 'base64');
                            } else if (result.audio) {
                                        // 如果直接返回binary数据
                                        audioData = Buffer.from(result.audio);
                            } else {
                                        throw new Error('RunPod TTS响应中未找到音频数据');
                            }
                  } else {
                            // 直接调用IndexTTS2 API
                            console.log('📡 直接调用IndexTTS2 API...');
    ...
    async generateVideo(params) {
          ...
          try {
                  ...
                  await dbRun(
                            `UPDATE projects SET video_url = ?, audio_url = ?, status = ?, completed_at = CURRENT_TIMESTAMP WHERE id = ?`,
